...
# ==============================
# Model: 4-layer Graph Transformer (L=4)
# widths: 512 -> 256 -> 128 -> 64  (concat=False, so out dim == listed width)
# ==============================
class TransformerNet(nn.Module):
    def __init__(self, num_node_features, output_dim):
        ...
        head_dim3 = 16   # 16*8 = 128
        head_dim4 = 8    #  8*8 =  64

        self.conv1 = TransformerConv(num_node_features, head_dim1 * HEADS, heads=HEADS, concat=False)
        self.conv2 = TransformerConv(head_dim1 * HEADS, head_dim2 * HEADS, heads=HEADS, concat=False)
        self.conv3 = TransformerConv(head_dim2 * HEADS, head_dim3 * HEADS, heads=HEADS, concat=False)
        self.conv4 = TransformerConv(head_dim3 * HEADS, head_dim4 * HEADS, heads=HEADS, concat=False)

        self.pool  = GlobalAttention(gate_nn=nn.Linear(head_dim4 * HEADS, 1))
        self.fc    = nn.Linear(head_dim4 * HEADS, output_dim)
        self.act   = nn.ELU()

    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        x = self.act(self.conv1(x, edge_index))  #  -> 512
        x = self.act(self.conv2(x, edge_index))  #  -> 256
        x = self.act(self.conv3(x, edge_index))  #  -> 128
        x = self.act(self.conv4(x, edge_index))  #  -> 64
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(subjects), start=1):
        print(f"\n===== Fold {fold}/{NUM_SUBJECTS} (L=4, H={HEADS}, RGB-only) =====")
        ...
        widths = [512, 256, 128, 64]  # L=4 => four stages
