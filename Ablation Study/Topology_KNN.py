...
# Graph/topology
K_NEIGHBORS     = 8
SYMMETRIC_EDGES = False   # if True, add both (i->j) and (j->i)
EPS = 1e-6
...
class TransformerNet(nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.conv1(x, ei); x = self.ln1(self.act(x))
        x = self.conv2(x, ei); x = self.ln2(self.act(x))
        x = self.conv3(x, ei); x = self.ln3(self.act(x))
...
def normalize_coords(coords_np: np.ndarray) -> np.ndarray:
    """Center to zero-mean and scale by RMS radius (translation/scale invariance)."""
...
def knn_edge_index(coords_np: np.ndarray, k: int = 8, symmetric: bool = False) -> torch.Tensor:
    """
    Build a kNN graph on normalized coords.
    Returns edge_index [2, E] (int64). NO edge attributes.
    """
    ...
    # indices of k nearest neighbors per node
...
def build_graphs_knn(df_landmarks, k=K_NEIGHBORS):
    ...
    for _, row in df_landmarks.iterrows():
        ...
        data = Data(x=x, edge_index=ei.long(), y=y)  # <-- NO edge_attr
...
def estimate_gflops_per_sample(n_nodes: int, n_edges: int, in_ch: int,
    ...
    """
    Coarse per-sample forward GFLOPs without edge_attr:
      - Q/K/V projections
    ...
    for w in widths:
        ...
        flops += 2.0 * (mac_linears + mac_dot + mac_weighted)
        fin = w
    last_dim = widths[-1] if widths else in_ch
    flops += 2.0 * (2.0 * n_nodes * last_dim)   # GlobalAttention (gate + weighted sum)
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(subjects), start=1):
        print(f"\n===== Fold {fold}/{NUM_SUBJECTS} (kNN ONLY, k={K_NEIGHBORS}, no edge_attr, H={HEADS}) =====")
